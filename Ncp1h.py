...
CREDENTIALS = {
    ...
    'telegram': {
        'token': '7897896851:AAFHIO_QGYU9bq5pxp79AfNp-MRxqwBdfvw',
        'chat_id': '5932647180'
    }
}
...
def fetch_data(symbol: str, interval: str = Config.INTERVAL, limit: int = Config.LIMIT) -> Optional[pd.DataFrame]:
    ...
    try:
        klines = client.futures_klines(symbol=symbol, interval=Client.KLINE_INTERVAL_1HOUR, limit=limit)
...
def detect_signals(data: pd.DataFrame, indicators: dict) -> Tuple[List[str], Optional[float], Optional[float], Optional[float]]:
    ...
    doji_threshold = (data['High'] - data['Low']).mean() * 0.15  # Increased from 0.1
    ...
    # Stochastic Signals (widened for 1h)
    if indicators['slowk'].iloc[-1] < 25 and indicators['slowd'].iloc[-1] < 25:
    ...
    elif indicators['slowk'].iloc[-1] > 75 and indicators['slowd'].iloc[-1] > 75:
